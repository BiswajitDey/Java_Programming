...
public class Gray {
	...
	// For use with Timer class. There should be labels.length+1 calls to the
	// Timer instance' now
	// method
	public static String[] labels = { "getRGB", "stream processing", "setRGB" };

	/**
	 * Serial program to convert color image to grayscale. Returns a Timer
	 * object with timing data collected during its execution.
	 * 
	 * @param image
	 * @param newImage
	 * @return
	...
	public static Timer gray_SS_FJ(FJBufferedImage image, FJBufferedImage newImage) {
		Timer time = new Timer(labels);
		/**
		 * Implement this function with serial stream processing and parallel
		 * setRGB and getRGB
		 */
		ColorModel colorModel = ColorModel.getRGBdefault();
		int w = image.getWidth();
	...
	public static Timer gray_PS_FJ(FJBufferedImage image, FJBufferedImage newImage) {
		Timer time = new Timer(labels);
		/**
		 * Implement this function with parallel stream processing and parallel
		 * setRGB and getRGB
		 */
		ColorModel colorModel = ColorModel.getRGBdefault();
		int w = image.getWidth();
	...
	public static Timer gray_PS(BufferedImage image, BufferedImage newImage) {
		Timer time = new Timer(labels);
		/**
		 * Implement this function with parallel stream processing and serial
		 * setRGB and getRGB
		 */
		ColorModel colorModel = ColorModel.getRGBdefault();
		int w = image.getWidth();
