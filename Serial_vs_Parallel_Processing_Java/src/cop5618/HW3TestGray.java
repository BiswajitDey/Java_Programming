 ...
 * 
 * This class can be run as an application or as JUnit tests. The application
 * runs the tests in a predetermined order; it is necessary to run testGray_SS()
 * before the other cases in order to compute speedup.
 * 
 ...
 * 
 * Properties warmupReps and reps can also be defined similarly, but have
 * default values if not defined. Examples:
 * 
 * -DwarmupReps="2" -Dreps="7"
 * 
 *
 */
public class HW3TestGray {
	...
	/**
	 * Adds the given prefix to the given filename. Example: old is
	 * "C:\Path\to\Pictures\hw3photo.jpg" prefix is "new_" returned String is
	 * "C:\Path\to\Pictures\new_hw3photo.jpg"
	 * 
	 * @param old
	 *            original filename
	...
	public void testGray_SS() throws IOException {
		...
		double[] meanDuration = new double[1]; // passed as one-element array so
												// it can be set in
												// statsToString.
	 ...
	 * . This test invokes the gray scale conversion with serial getRGB and
	 * setRGB and parallel stream processing. The speedup is compared to the
	 * duration from testGray_SS, or 0 if that test case has not run.
	 * 
	 * @throws IOException
	 */
	@Test
	 ...
	 * . This test invokes the gray scale conversion with parallel getRGB and
	 * setRGB and serial stream processing. The speedup is compared to the
	 * duration from testGray_SS, or 0 if that test case has not run.
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * Allows the test cases to be invoked as an application without the JUnit
	 * test framework in a controlled order.
	 * 
	 * @param args
	 * @throws Exception
	 */
